    ...
    sp<AudioPolicyEffects>audioPolicyEffects;
    {
        ...
        if (status == NO_ERROR) {
            ...
            case AudioPolicyInterface::API_INPUT_MIX_CAPTURE:
                if (!isAudioServerOrMediaServerOrSystemServerOrRootUid(IPCThreadState::self()->getCallingUid()) && !canCaptureOutput) {
...
Status AudioPolicyService::startInput(int32_t portIdAidl)
{
    ...
    // check calling permissions
    if (!isAudioServerOrMediaServerOrSystemServerOrRootUid(IPCThreadState::self()->getCallingUid()) && !(startRecording(client->attributionSource, String16(msg.str().c_str()),
